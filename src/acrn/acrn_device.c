/*
 * acrn_device.c: acrn device management
 * Adapted from bhyve_device.c. Original license and copyright:
 *
...
static int
acrnCollectPCIAddress(virDomainDef *def G_GNUC_UNUSED,
...
acrnDomainPCIAddressSetCreate(virDomainDef *def, unsigned int nbuses)
{
...
static int
acrnAssignDevicePCISlots(virDomainDef *def,
                          virDomainPCIAddressSet *addrs)
{
    size_t i;
...
int acrnDomainAssignPCIAddresses(virDomainDef *def,
                                  virDomainObj *obj)
{
    virDomainPCIAddressSet *addrs = NULL;
    acrnDomainObjPrivate *priv = NULL;

    if (!(addrs = acrnDomainPCIAddressSetCreate(def, 1)))
        return -1;

    if (acrnAssignDevicePCISlots(def, addrs) < 0)
        return -1;
...
{
    return acrnDomainAssignPCIAddresses(def, obj);
}
