...
static virCommand *
virAcrnProcessBuildAcrnloadCmd(virDomainDef *def, virDomainDiskDef *disk)
{
    virCommand *cmd;

    cmd = virCommandNew(ACRNLOAD);

    if (def->os.bootloaderArgs == NULL) {
        VIR_DEBUG("acrnload with default arguments");
        ...
        VIR_DEBUG("acrnload with arguments");
...
static virCommand *
virAcrnProcessBuildCustomLoaderCmd(virDomainDef *def)
{
    virCommand *cmd;
...
static bool
virAcrnUsableDisk(virDomainDiskDef *disk)
{
    if (virDomainDiskTranslateSourcePool(disk) < 0)
        return false;
...
static void
virAcrnFormatGrubDevice(virBuffer *devicemap, virDomainDiskDef *def)
{
...
static virCommand *
virAcrnProcessBuildGrubacrnCmd(virDomainDef *def,
                                 struct _acrnConn *driver,
                                 ...
                                 char **devicesmap_out)
{
    ...
    if (def->os.bootloaderArgs != NULL)
        return virAcrnProcessBuildCustomLoaderCmd(def);
    ...
    cd = hdd = userdef = NULL;
    for (i = 0; i < def->ndisks; i++) {
        if (!virAcrnUsableDisk(def->disks[i]))
            continue;
    ...
    VIR_DEBUG("grub-acrn with default arguments");
    ...
    if (devicesmap_out != NULL) {
        ...
        if (userdef != NULL) {
            virAcrnFormatGrubDevice(&devicemap, userdef);
        } else {
            if (hdd != NULL)
                virAcrnFormatGrubDevice(&devicemap, hdd);

            if (cd != NULL)
                virAcrnFormatGrubDevice(&devicemap, cd);
        }
    ...
    if ((acrnDriverGetGrubCaps(driver) & ACRN_GRUB_CAP_CONSDEV) != 0 &&
        def->nserials > 0) {
        virDomainChrDef *chr;
...
virAcrnGetBootDisk(virDomainDef *def)
{
    size_t i;
    ...
    if (boot_dev != -1) {
         ...
         * is set for individual device. However, as acrn does not support
                            ...
                            const char *devmap_file, char **devicesmap_out)
{
    ...
    if (def->os.bootloader == NULL) {
        disk = virAcrnGetBootDisk(def);

        if (disk == NULL)
            return NULL;

        return virAcrnProcessBuildAcrnloadCmd(def, disk);
    } else if (strstr(def->os.bootloader, "grub-acrn") != NULL) {
        return virAcrnProcessBuildGrubacrnCmd(def, driver, devmap_file,
                                                devicesmap_out);
    } else {
        return virAcrnProcessBuildCustomLoaderCmd(def);
    }
