/*
 * acrn_firmware.c: acrn firmware management
 * Adapted from bhyve_firmware.c. Original license and copyright:
 *
                        ...
                        unsigned int flags)
{
    ...
    g_autoptr(virAcrnDriverConfig) cfg = virAcrnDriverGetConfig(driver);
    ...
    if (virDirOpenIfExists(&dir, firmware_dir) > 0) {
        while ((virDirRead(dir, &entry, firmware_dir)) > 0) {
            ...
            if (STREQ(entry->d_name, ACRN_DEFAULT_FIRMWARE)) {
