/*
 * acrn_parse_command.c: Acrn command parser
...
VIR_LOG_INIT("acrn.acrn_parse_command");
 ...
 */
static char *
acrnParseCommandLineUnescape(const char *command)
{
...
/*
 * Try to extract loader and acrn argv lists from a command line string.
 */
static int
acrnCommandLineToArgv(const char *nativeConfig,
                      int *loader_argc,
                      char ***loader_argv,
                      int *acrn_argc,
                      char ***acrn_argv)
{
    ...
    char **_acrn_argv = NULL;
    char **_loader_argv = NULL;

    nativeConfig_unescaped = acrnParseCommandLineUnescape(nativeConfig);
    ...
    for (i = 0; i < line_count; i++) {
         ...
         * problem, since usually no multiple loaders or acrnrun commands
        ...
        if (!_acrn_argv && STREQ(arglist[0], "/usr/sbin/acrn")) {
            VIR_REALLOC_N(_acrn_argv, args_count + 1);
            if (!acrn_argc)
                goto error;
            for (j = 0; j < args_count; j++)
                _acrn_argv[j] = g_steal_pointer(&arglist[j]);
            _acrn_argv[j] = NULL;
            *acrn_argc = args_count-1;
...
static int
acrnParseAcrnLPCArg(virDomainDef *def,
...
static int
acrnParsePCISlot(const char *slotdef,
                  unsigned *bus,
                  unsigned *slot,
                  unsigned *function)
{
...
static int
acrnParsePCIDisk(virDomainDef *def,
                  unsigned caps G_GNUC_UNUSED,
                  unsigned pcibus,
                  unsigned pcislot,
                  unsigned function,
...
static int
acrnParsePCINet(virDomainDef *def,
...
static int
acrnParsePCIFbuf(virDomainDef *def,
                  virDomainXMLOption *xmlopt,
                  unsigned caps G_GNUC_UNUSED,
                  unsigned bus,
                  unsigned slot,
        ...
        if (STRPREFIX(param, "rfb=") || STRPREFIX(param, "tcp=")) {
            ...
            *separator = '\0';
            ...
            if (separator != param)
                ...
                /* Default to 127.0.0.1, just like acrn does */
...
static int
acrnParseAcrnPCIArg(virDomainDef *def,
                      virDomainXMLOption *xmlopt,
                      unsigned caps,
                      unsigned *nvirtiodisk,
                      unsigned *nahcidisk,
                      const char *arg)
{
    ...
    if (acrnParsePCISlot(slotdef, &bus, &slot, &function) < 0)
        goto error;

    if (STREQ(emulation, "ahci-cd"))
        acrnParsePCIDisk(def, caps, bus, slot, function,
...
/*
 * Parse the /usr/sbin/acrn command line.
 */
static int
acrnParseAcrnCommandLine(virDomainDef *def,
                           virDomainXMLOption *xmlopt,
                           unsigned caps,
                           int argc, char **argv)
{
    ...
    for (opti = 1; opti < argc; opti++) {
        ...
        case 'l':
            CONSUME_ARG(arg);
            if (acrnParseAcrnLPCArg(def, caps, arg))
                return -1;
            break;
        case 's':
            CONSUME_ARG(arg);
        ...
        case 'm':
            CONSUME_ARG(arg);
            if (acrnParseMemsize(arg, &memory)) {
                virReportError(VIR_ERR_OPERATION_FAILED, "%s",
    ...
    if (argc != opti) {
        virReportError(VIR_ERR_OPERATION_FAILED, "%s",
                       _("Failed to parse arguments for acrn command"));
        return -1;
    }
    ...
    if (def->name == NULL) {
        ...
        /* the vm name of the loader and the acrnrun command differ, throw an
         * error here */
        virReportError(VIR_ERR_OPERATION_FAILED, "%s",
...
/*
 * Parse the /usr/sbin/acrnload command line.
 */
static int
acrnParseAcrnLoadCommandLine(virDomainDef *def,
                               int argc, char **argv)
{
    /* acrnload called with default arguments when only -m and -d are given.
...
static int
acrnParseCustomLoaderCommandLine(virDomainDef *def,
...
virDomainDef *
acrnParseCommandLineString(const char* nativeConfig,
                            unsigned caps,
                            virDomainXMLOption *xmlopt)
{
    g_autoptr(virDomainDef) def = NULL;
    int acrn_argc = 0;
    g_auto(GStrv) acrn_argv = NULL;
    int loader_argc = 0;
    ...
    /* Initialize defaults. */
    def->virtType = VIR_DOMAIN_VIRT_ACRN;
    ...
    if (acrnCommandLineToArgv(nativeConfig,
    ...
    if (acrnParseAcrnCommandLine(def, xmlopt, caps, acrn_argc, acrn_argv))
        return NULL;
    if (loader_argv && STREQ(loader_argv[0], "/usr/sbin/acrnload")) {
        if (acrnParseAcrnLoadCommandLine(def, loader_argc, loader_argv))
            return NULL;
    } else if (loader_argv) {
        if (acrnParseCustomLoaderCommandLine(def, loader_argc, loader_argv))
            return NULL;
    }
